//The 6502 cpu specs can be found here: https://www.nesdev.org/obelisk-6502-guide/reference.html
...
impl CPU {
    ...
    fn get_op_address(&mut self, mode: &AddressingMode) -> (u16, bool) {
    ...
    fn lda(&mut self, mode: &AddressingMode) {
        let (addr, page_cross) = self.get_op_address(&mode);
    ...
    fn sta(&mut self, mode: &AddressingMode) {
        let (addr,_) = self.get_op_address(mode);
    ...
    fn compare(&mut self, mode: &AddressingMode, compare_with: u8) {
        ...
        self.update_zero_and_negative(compare_with.wrapping_sub(data));
        ...
        F: FnMut(&mut CPU),
    {
        ...
        loop {
            ...
            match code {
                ...
                /* STX */
                0x86 | 0x96 | 0x8e => {
                    let (addr,_) = self.get_op_address(&opcode.mode);
                ...
                0xc7 | 0xd7 | 0xCF | 0xdF | 0xdb | 0xd3 | 0xc3 => {
                    ...
                    data = data.wrapping_sub(1);
                    self.mem_write(addr, data);
                    // self._update_zero_and_negative(data);
                    if data <= self.register_a {
                        self.status.insert(CpuFlags::CARRY);
                    }
                    self.update_zero_and_negative(self.register_a.wrapping_sub(data));
                }
                ...
                | 0x3c | 0x5c | 0x7c | 0xdc | 0xfc => {
                    let (addr, page_cross) = self.get_op_address(&opcode.mode);
                    let data = self.mem_read(addr);
                ...
                0x9b => {
                    ...
                    self.stack_pointer = data;
                    let mem_address =
                        self.mem_read_u16(self.pc) + self.register_y as u16;
                ...
                0x9c => {
                    let mem_address =
                        self.mem_read_u16(self.pc) + self.register_x as u16;
